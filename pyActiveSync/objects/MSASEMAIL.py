...
class email_Flag(object):                   #http://msdn.microsoft.com/en-us/library/ee160518(v=exchg.80).aspx
    ...
    def marshal(self):
        import base64
        return "%s//%s//%s//%s//%s//%s//%s//%s//%s//%s//%s//%s//%s" % (self.tasks_Subject, str(self.email_flag_Status), self.email_FlagType, self.tasks_DateCompleted, self.email_CompleteTime, self.tasks_StartDate,
